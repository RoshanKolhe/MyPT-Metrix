...
export default function KpiListView() {
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      try {
        // Make API call to delete the kpi
        const response = await axiosInstance.delete(`/kpi/${id}`);
        if (response.status === 204) {
          console.log('Kpi deleted successfully');
          enqueueSnackbar('Kpi Deleted Successfully');
          confirm.onFalse();
          refreshKpis();
        }
      } catch (error) {
        console.error('Error deleting kpi:', error.response?.data || error.message);
        enqueueSnackbar(typeof error === 'string' ? error : error.error.message, {
          variant: 'error',
        });
      }
    },
    [confirm, enqueueSnackbar, refreshKpis]
  );
  ...
  const handleViewRow = useCallback(
    (id) => {
      router.push(paths.dashboard.kpi.view(id));
    },
    [router]
  );
      ...
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'Kpi Management', href: paths.dashboard.kpi.list },
            { name: 'List' },
          ]}
          sx={{
            mb: { xs: 3, md: 5 },
