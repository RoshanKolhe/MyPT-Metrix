...
export default function KpiQuickEditForm({
  currentKpi,
  open,
  onClose,
  refreshKpis,
  ...
  const defaultValues = useMemo(
    () => ({
      hsnCode: currentKpi?.hsnCode || '',
      tax: currentKpi?.tax || '',
      status: currentKpi?.status ? 1 : 0,
    }),
    [currentKpi]
  );
  ...
  const onSubmit = handleSubmit(async (formData) => {
    ...
    try {
      ...
      await axiosInstance.patch(`/hsn-masters/${currentKpi.id}`, inputData);
      refreshKpis();
      reset();
      onClose();
