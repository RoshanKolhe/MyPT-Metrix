...
export default function ConductionNewEditForm({ currentConduction }) {
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      isActive: currentConduction?.isActive ?? 1,
      ...
      reportingUser: currentConduction?.supervisor || null,
    }),
    [currentConduction]
  );
  ...
  useEffect(() => {
    ...
    const isEditing = !!currentConduction?.id;

    if (branchId && departmentId) {
      fetchReportingUsers(branchId, departmentId).then(() => {
        if (isEditing && currentConduction?.supervisor) {
          setValue('reportingUser', currentConduction.supervisor);
        }
      });
    } else {
      setReportingUserOptions([]);
  ...
  }, [branch, department, setValue, currentConduction]);
  ...
  }, [branch, currentConduction, setValue]);
          ...
          <Card sx={{ p: 3 }}>
            ...
            <Stack alignItems="flex-end" sx={{ mt: 3 }}>
              <LoadingButton type="submit" variant="contained" loading={isSubmitting}>
                {!currentConduction ? 'Create Conduction' : 'Save Changes'}
              </LoadingButton>
            </Stack>
          </Card>
        </Grid>
