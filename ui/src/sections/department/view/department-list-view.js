...
export default function DepartmentListView() {
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      try {
        // Make API call to delete the department
    ...
    [confirm, enqueueSnackbar, refreshDepartments]
  );
  ...
  const handleViewRow = useCallback(
    (id) => {
      router.push(paths.dashboard.department.view(id));
    },
    [router]
  );
    ...
    <>
      ...
      {quickEdit.value && quickEditRow && (
        <DepartmentQuickEditForm
          currentDepartment={quickEditRow}
