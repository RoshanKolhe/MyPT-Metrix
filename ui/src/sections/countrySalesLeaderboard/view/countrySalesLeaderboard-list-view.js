...
//
import { useGetCountrySalesLeaderboardsWithFilter } from 'src/api/countrySalesLeaderboard';
...
export default function CountrySalesLeaderboardListView({ filter }) {
  console.log(filter);
  ...
  const {
    filteredCountrySalesLeaderboards: countrySalesLeaderboards,
    countrySalesLeaderboardsLoading,
    countrySalesLeaderboardsEmpty,
    refreshFilterCountrySalesLeaderboards,
  } = useGetCountrySalesLeaderboardsWithFilter(filter);
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      try {
        // Make API call to delete the countrySalesLeaderboard
        const response = await axiosInstance.delete(`/countrySalesLeaderboard/${id}`);
        if (response.status === 204) {
          console.log('CountrySalesLeaderboard deleted successfully');
          enqueueSnackbar('CountrySalesLeaderboard Deleted Successfully');
          confirm.onFalse();
          refreshFilterCountrySalesLeaderboards();
        }
      } catch (error) {
        console.error(
          'Error deleting countrySalesLeaderboard:',
    ...
    [confirm, enqueueSnackbar, refreshFilterCountrySalesLeaderboards]
  );
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.countrySalesLeaderboard.edit(id));
    },
    [router]
  );
  ...
  const handleViewRow = useCallback(
    (id) => {
      router.push(paths.dashboard.countrySalesLeaderboard.view(id));
    },
    [router]
  );
  ...
  const handleExport = useCallback(() => {
    const fileName = 'countrySalesLeaderboard.xlsx';
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'CountrySalesLeaderboards');
    XLSX.writeFile(wb, fileName);
  }, [dataFiltered]);

  useEffect(() => {
    if (countrySalesLeaderboards) {
      // const updatedCountrySalesLeaderboards = countrySalesLeaderboards.filter((obj) => !obj.permissions.includes('super_admin'));
      setTableData(countrySalesLeaderboards);
    }
  }, [countrySalesLeaderboards]);
              ...
              <Table size={table.dense ? 'small' : 'medium'}>
                ...
                <TableBody>
                  {dataFiltered
                    ...
                    .map((row) => (
                      <CountrySalesLeaderboardTableRow
                        key={row.id}
                        row={row}
                        ...
                        handleQuickEditRow={(countrySalesLeaderboard) => {
                          handleQuickEditRow(countrySalesLeaderboard);
                        }}
                        quickEdit={quickEdit}
                      />
...
CountrySalesLeaderboardListView.propTypes = {
