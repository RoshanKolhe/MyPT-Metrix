...
export default function SaleNewEditForm({ currentSale }) {
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      isActive: currentSale?.isActive ?? 1,
      ...
      reportingUser: currentSale?.supervisor || null,
    }),
    [currentSale]
  );
  ...
  useEffect(() => {
    ...
    if (branchId && departmentId) {
      fetchReportingUsers(branchId, departmentId).then(() => {
        if (isEditing && currentSale?.supervisor) {
          setValue('reportingUser', currentSale.supervisor);
        }
      });
    } else {
      setReportingUserOptions([]);
  ...
  }, [branch, department, setValue, currentSale]);
          ...
          <Card sx={{ p: 3 }}>
            ...
            <Stack alignItems="flex-end" sx={{ mt: 3 }}>
              <LoadingButton type="submit" variant="contained" loading={isSubmitting}>
                {!currentSale ? 'Create Sale' : 'Save Changes'}
              </LoadingButton>
            </Stack>
          </Card>
        </Grid>
